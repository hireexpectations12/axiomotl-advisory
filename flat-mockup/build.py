...
fonts = re.sub(r'url\((type-\d+\.ttf)\)', lambda match: 'url(data:font/ttf;base64,' + base64.b64encode((source / 'assets' / match[1]).read_bytes()).decode() + ')', fonts)
...
scripts = []
for filename in ['vendor/gsap.min.js', 'vendor/ScrollTrigger.min.js', 'app.js']:
...
output = source.parent / 'axiomotl-flat-mockup.html'
