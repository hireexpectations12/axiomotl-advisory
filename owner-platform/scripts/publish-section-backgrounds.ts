...
for (const id of ["services", "decision-brief", "method", "about", "contact"])
  if ($(`section#${id}`).length !== 1)
    throw new Error(`Missing section: ${id}`);
const css = await readFile(
  new URL("../design/combined/section-backgrounds.css", import.meta.url),
  "utf8",
);
if (home.css.includes("/* Live section backgrounds requested 2026-09-21. */"))
  throw new Error(
...
await writeFile(
  new URL(`section-backgrounds-${Date.now()}.json`, backupDir),
...
if (!process.argv.includes("--publish")) {
  console.log({ status: "validated", version: draft.version, sections: 5 });
  process.exit(0);
}
