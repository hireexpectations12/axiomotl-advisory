...
const css = await readFile(
  new URL("../design/combined/hero-biography-refinement.css", import.meta.url),
  "utf8",
);
if (home.css.includes("/* Hero CTA alignment and justified practitioner biography. */"))
  throw new Error(
...
const bytes = await readFile(new URL("../design/combined/Design-teal.png",import.meta.url));
const iconKey = `${site}/design-teal-${Date.now()}.png`;
...
await writeFile(
  new URL(`hero-biography-refinement-${Date.now()}.json`, backupDir),
...
const asset = await db.from("assets").insert({site_id:site,object_key:iconKey,url:iconUrl,name:"Design-teal.png",size:bytes.length});
