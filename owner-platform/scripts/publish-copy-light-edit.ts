...
const edits = JSON.parse(await readFile(new URL("../design/combined/copy-light-edit.json", import.meta.url), "utf8"));
for (const edit of edits) {
  if (home.html.split(edit.before).length !== 2)
    throw new Error("Live copy changed; reconcile before editing.");
  home.html = home.html.replace(edit.before, edit.after);
}
const document = validateDocument(next);
const backupDir = new URL("../../outputs/release-backups/", import.meta.url);
await mkdir(backupDir, { recursive: true });
await writeFile(
  new URL(`copy-light-edit-${Date.now()}.json`, backupDir),
  JSON.stringify({ draft, publication: publication.data }, null, 2),
);
await writeFile(
  new URL(
    "../../outputs/slopmonster-live/proposed-document.json",
    import.meta.url,
  ),
  JSON.stringify(document, null, 2),
);
if (!process.argv.includes("--publish")) {
  console.log({ status: "validated", version: draft.version, sections: 2 });
  process.exit(0);
}
