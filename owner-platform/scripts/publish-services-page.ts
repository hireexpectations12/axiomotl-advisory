...
if (JSON.stringify(draft.document) !== JSON.stringify(publication.data) && !process.argv.includes("--use-published"))
...
const css = await readFile(
  new URL("../design/services/styles.css", import.meta.url),
  "utf8",
);
const document = addServicesPage(publication.data, content, css);
const servicesPage = document.pages.find((page) => page.path === "/services")!;
await writeFile(
  new URL("../public/services-review.html", import.meta.url),
  renderPage(document, { ...servicesPage, noIndex: true }),
);
await mkdir(new URL("../../outputs/services-page/", import.meta.url), {
  recursive: true,
});
const backupDir = new URL("../../outputs/release-backups/", import.meta.url);
await mkdir(backupDir, { recursive: true });
await writeFile(
  new URL(`services-page-${Date.now()}.json`, backupDir),
