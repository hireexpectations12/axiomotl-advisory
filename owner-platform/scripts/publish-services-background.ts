...
const page = next.pages.find((page: { path: string }) => page.path === "/services");
if (!page || page.project) throw new Error("Services page unavailable or editor structure changed.");
const before = ".svc-intro {\n  background: #481e72;";
const after = ".svc-intro {\n  background: #21132f;";
if (page.css.split(before).length !== 2) throw new Error("Services background changed; inspect before editing.");
page.css = page.css.replace(before, after);
const document = validateDocument(next);
const backupDir = new URL("../../outputs/release-backups/", import.meta.url);
await mkdir(backupDir, { recursive: true });
await writeFile(
  new URL(`services-background-${Date.now()}.json`, backupDir),
