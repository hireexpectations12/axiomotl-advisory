...
for (const id of ["sectors", "decision-brief"])
  if ($(`section#${id}`).length !== 1)
    throw new Error(`Missing section: ${id}`);
const css = await readFile(
  new URL("../design/combined/industry-background.css", import.meta.url),
  "utf8",
);
if (home.css.includes("/* Industry background matched to decision brief 2026-09-21. */"))
  throw new Error(
...
await writeFile(
  new URL(`industry-background-${Date.now()}.json`, backupDir),
