...
`design/combined/refinements.css` follows the palette styles. It removes legacy service minimum heights, compacts service rows and engagement steps, and justifies paragraphs with automatic hyphenation. Industry experience uses three editorial rows preserving existing practitioner capability statements. The canonical journey form now mounts inline in `finder.html`; the decision brief remains separate. `hero-blend.css` follows refinements to soften the hero artwork edges. The renderer loads `public/runtime/polish.css` after document styles, and the sandbox preview embeds the same stylesheet.
...
Both `publish-refinements.ts` and `publish-hero-blend.ts` prepare a local review candidate by default; `--publish` requires the reviewed candidate and draft version to match. Both reject repeat application. The hero blend additionally refuses an existing editor snapshot, preserving it for reconciliation. Do not rerun these one-time publishers to deploy application code. The hero blend is already published in revision 54.

Runtime polish adds a responsive light sweep over the existing hero artwork, visible engagement-step lights and link cues using the shared motion controls. Reduced-motion and pause behavior remain supported. Recognized service conversation actions point to `#contact` when that section exists; contact delivery remains disabled. Release validation passed all 83 unit tests, 33 browser tests, TypeScript checks and the production build; production deployment verification is a separate release step.
