...
export function FormEditor({
  ...
  return (
    <div className="stack forms-workspace">
      <div className="row forms-toolbar">
        <label className="field">
          <span>Form</span>
          <select
            value={form?.id ?? ""}
      ...
      {!form && (
        <div className="panel">
          ...
          <p>Add a form to start writing questions and results.</p>
        </div>
      )}
      {form && (
        <div key={form.id} className="stack">
          <div className="forms-overview">
            <h2>{form.name}</h2>
            <p className="muted">
              {form.questions.length} questions · {form.outcomes.length} results
            </p>
          </div>
          <nav className="forms-navigation" aria-label="Form editor sections">
            {[
              ["questions", "Questions"],
              ["results", "Results"],
              ["rules", "Rules"],
              ...
              <div className="form-grid">
                ...
                <label className="field">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      update({ startQuestionId: e.target.value })
                    }
                  >
                    {!form.questions.some(
          ...
          {(validation.errors.length > 0 || validation.warnings.length > 0) && (
            <div className="notice" role="status">
              ...
              <p>Check these items before publishing your form.</p>
              <ul>
          ...
          {view === "questions" && (
            <section className="stack">
              ...
              <p className="muted">
                Open a question to edit its wording and answers.
              </p>
                      ...
                      {!canChangeQuestionType(form, q.id) && (
                        <p id={`${q.id}-type-help`} className="notice">
                          ...
                          {form.rules
                            ...
                            .join(", ")}
                          . Then recreate the conditions for the new answer
                          type.
                        </p>
                      )}
                      <label className="row">
                      ...
                      {q.kind === "text" ? (
                        ...
                        <>
                          {q.choices.map((choice, ci) => (
                            <div className="forms-answer stack" key={choice.id}>
                              <Field
                                label={`Answer ${ci + 1}`}
                                value={choice.label}
                                onChange={(label) =>
          ...
          {view === "rules" && (
            <section className="stack">
              <h2>Rules</h2>
              <p className="muted">
                Choose where answers lead. The first matching rule is used.
              </p>
          ...
          {view === "results" && (
            <section className="stack">
              <h2>Results</h2>
              <p className="muted">
                What visitors see when they finish the form.
              </p>
                  ...
                  <details className="panel" key={outcome.id}>
                    ...
                    <div className="stack">
                      ...
                      <Order
                        ...
                        onMove={(direction) =>
                          update({
                            outcomes: move(form.outcomes, oi, direction),
                          })
                        }
                        onRemove={() =>
                          update({
            ...
            <section className="panel stack">
              ...
              {trace && (
                <div className="notice">
                  ...
                  <p>
                    Outcome:{" "}
                    {form.outcomes.find((o) => o.id === trace.outcomeId)
